...
public:
     ...
     */
    //as we learned at lecture, we must override default destructor if we have pointer fields
    ~PointerWrapper() {
        delete ptr;
    }
    ...
    /**
     * as we can see no memory was allocated. only deleted.
     * i didnt understand it completely yet but it seems that it happens only from outside the wrapper.\
     * need to check.
     */
    ...
    T* release() {
        T* copy = ptr;
        ptr = nullptr; //now the wrapper is empty
        return copy;
    }
     ...
     */
     ...
     //const no change variable
     //no except no exception throw
